...
def get_dataloaders(
    ...
    def pick(split, n):
        split = ds[split] if n == -1 else ds[split].select(range(n))
        return split.map(lambda x: x["translation"], remove_columns=["translation"])

    train_raw = pick("train", num_examples)
    valid_raw = pick("validation", num_examples // 10)
    test_raw = pick("test", num_examples // 10)
    ...
    def tokenize_fn(examples):
        ...
        return {
            ...
            "tgt": [[SOS_ID] + x.ids[:-1] for x in targets_enc],
